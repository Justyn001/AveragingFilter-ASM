...
extern "C" int _fastcall ApplyAverageFilter(unsigned char* input, unsigned char* output, int width, int height, int radius);

// Funkcja filtru uœredniaj¹cego
...
// Funkcja stosuj¹ca filtr i zapisuj¹ca wynik
...
int _tmain(int argc, _TCHAR* argv[]) {
    ...
    // Pole tekstowe z domyœln¹ œcie¿k¹
    ...
    std::string basePath = "D:\\Informatyka\\JA\\proba2\\JASol\\";
    std::string fileName = "";
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            // Obs³uga pola tekstowego (dodawanie tekstu)
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (buttonAsm.getGlobalBounds().contains(mousePos.x, mousePos.y)) {
                    ...
                    if (inputImage.loadFromFile(fullPath)) {
                        ...
                        // Przygotowanie obrazu wyjœciowego
                        outputImage.create(inputImage.getSize().x, inputImage.getSize().y);

                        // Wywo³anie funkcji assemblerowej
